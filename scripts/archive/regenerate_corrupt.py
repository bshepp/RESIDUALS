...
def parse_filename(filename: str) -> tuple:
    ...
    for method_name in DECOMPOSITION_REGISTRY:
        if decomp_str.startswith(method_name):
            ...
            if param_str:
                decomp_params = parse_params(
                    param_str, DECOMPOSITION_REGISTRY[method_name].param_ranges
                )
            break
    ...
    for method_name in UPSAMPLING_REGISTRY:
        if upsample_str.startswith(method_name):
            ...
            if param_str.startswith("_"):
                param_str = param_str[1:]
            if param_str:
                upsample_params = parse_params(
                    param_str, UPSAMPLING_REGISTRY[method_name].param_ranges
                )
            break
