...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Recompute all checksums (ignore existing)"
    )
...
def write_manifest(manifest_path: Path, checksums: dict, source_dir: Path):
    ...
    with open(manifest_path, "w") as fp:
        fp.write("# RESIDUALS Exhaustive Run - SHA256 Checksums\n")
        fp.write(f"# Generated: {datetime.now().isoformat()}\n")
        fp.write(f"# Total files: {len(checksums)}\n")
        fp.write(f"# Source: {source_dir}\n")
        fp.write("#\n")
        fp.write("# Format: SHA256_HASH  FILENAME\n")
        fp.write("# Verify with: sha256sum -c CHECKSUMS.txt\n")
        fp.write("#\n")
