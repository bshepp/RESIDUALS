...
def find_near_duplicates_by_fingerprint(fingerprints: dict, threshold: float = 0.999) -> list:
            ...
            for ci in range(chunk.shape[0]):
                for cj in range(other.shape[0]):
                    ...
                    if sims[ci, cj] >= threshold:
                        near_dupes.append((names[global_i], names[global_j], float(sims[ci, cj])))

    return sorted(near_dupes, key=lambda x: -x[2])


def sample_correlation_analysis(results_dir: Path, pairs: list, n_samples: int = 100) -> list:
...
def analyze_parameter_sensitivity(fingerprints: dict) -> dict:
    ...
    return {"decomposition": decomp_sensitivity, "upsampling": upsamp_sensitivity}
...
def generate_report(
    ...
    with open(output_path, "w", encoding="utf-8") as fp:
        ...
        fp.write("- **Near-duplicate pairs** (fingerprint similarity > 0.999): ")
...
def save_json_results(
    ...
    results = {
        ...
        "near_duplicates": [
            {"file1": f1, "file2": f2, "similarity": sim} for f1, f2, sim in near_dupes
        ],
        "correlations": [
...
def main():
    ...
    parser.add_argument("--fast", action="store_true", help="Skip correlation sampling (faster)")
    parser.add_argument(
        "--fingerprint-limit",
        type=int,
        default=None,
    ...
    parser.add_argument(
        "--correlation-samples", type=int, default=100, help="Number of pairs to correlate"
    )
    ...
    parser.add_argument(
        "--checkpoint", type=str, default=None, help="Checkpoint file for resumable fingerprinting"
    )
    parser.add_argument(
        "--gc-interval", type=int, default=100, help="Run garbage collection every N files"
    )
    ...
    if checksums_path.exists():
        ...
        print("  Warning: Checksums file not found, skipping exact duplicate detection")
        checksums = {}
        exact_dupes = {}
    ...
    near_dupes = find_near_duplicates_by_fingerprint(
        fingerprints, threshold=args.near_dupe_threshold
    )
    ...
    if not args.fast and near_dupes:
        ...
        correlations = sample_correlation_analysis(
            results_dir, near_dupes, n_samples=args.correlation_samples
        )
    ...
    # Generate outputs
    print("\nGenerating reports...")
    generate_report(exact_dupes, near_dupes, correlations, sensitivity, clusters, output_path)

    json_path = output_path.with_suffix(".json")
    save_json_results(exact_dupes, near_dupes, correlations, sensitivity, clusters, json_path)
