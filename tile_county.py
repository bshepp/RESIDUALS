...
def compute_svf_openness(dem, n_dir=32, radius=50):
    ...
    for d_idx, theta in enumerate(angles):
        ...
        for r in range(1, radius + 1):
            ...
            shifted = padded[radius + di: radius + di + h,
...
def compute_grid(tile_gdf: gpd.GeoDataFrame) -> list:
    ...
    logger.info(f"County extent: X=[{county_x_min:.0f}, {county_x_max:.0f}], "
                f"Y=[{county_y_min:.0f}, {county_y_max:.0f}]")
    logger.info(f"County size: {county_x_max - county_x_min:.0f} x "
                f"{county_y_max - county_y_min:.0f} ft")

    n_cols = int(np.ceil((county_x_max - county_x_min) / TILE_SIZE))
    n_rows = int(np.ceil((county_y_max - county_y_min) / TILE_SIZE))

    logger.info(f"Grid: {n_rows} rows x {n_cols} cols = {n_rows * n_cols} tiles "
...
def render_tile_png(dem, residual, svf, pos_openness, neg_openness,
                    tile_id, output_path, sites_in_tile=None):
    """
    ...
    try:
        _trend_fine, residual_fine = run_decomposition(
            'tophat', dem, {'size': 3, 'mode': 'white'})
    except Exception:
    ...
    def mark_sites(ax):
        if sites_in_tile:
            for _name, px_x, px_y in sites_in_tile:
                ax.plot(px_x, px_y, '+', color='lime', markersize=12,
                        markeredgewidth=1.5)
    ...
    # --- Multi-directional hillshade ---
    ax = axes[0, 0]
    ax.imshow(mdhs, cmap='gray', origin='lower', aspect='equal',
              interpolation='none')
    ax.set_title('Multi-Dir Hillshade (16 dir)', fontsize=12,
                 fontweight='bold')
    ax.set_ylabel('Visualization', fontsize=13, fontweight='bold',
    ...
    # --- Sky-View Factor ---
    ax = axes[0, 1]
    ax.imshow(svf, cmap='gray', origin='lower', aspect='equal',
              interpolation='none', vmin=0.7, vmax=1.0)
    ...
    # --- Slope ---
    ax = axes[0, 3]
    ax.imshow(slope, cmap='gray', origin='lower', aspect='equal',
    ...
    ax.imshow(pos_openness, cmap='gray', origin='lower', aspect='equal',
              interpolation='none', vmin=p01, vmax=p99)
    ax.set_title('Positive Openness (Yokoyama 2002)', fontsize=12,
                 fontweight='bold')
    ax.set_ylabel('Terrain Shape', fontsize=13, fontweight='bold',
                  rotation=90, labelpad=10)
    strip_axes(ax)
    mark_sites(ax)
    ...
    ax.imshow(neg_openness, cmap='gray', origin='lower', aspect='equal',
              interpolation='none', vmin=p01, vmax=p99)
    ax.set_title('Negative Openness (Yokoyama 2002)', fontsize=12,
                 fontweight='bold')
    strip_axes(ax)
    mark_sites(ax)
    ...
    # --- SLRM ---
    ax = axes[1, 2]
    im_slrm = ax.imshow(slrm, cmap='hot', origin='lower', aspect='equal',
                        vmin=0, vmax=shared_vmax, interpolation='none')
    ax.set_title('SLRM (Hesse 2010) r=50ft', fontsize=12,
                 fontweight='bold')
    strip_axes(ax)
    mark_sites(ax)
    ...
    # --- TPI ---
    ax = axes[1, 3]
    im_tpi = ax.imshow(tpi, cmap='hot', origin='lower', aspect='equal',
                       vmin=0, vmax=shared_vmax, interpolation='none')
    ax.set_title('TPI (Weiss 2001) r=50ft', fontsize=12,
                 fontweight='bold')
    strip_axes(ax)
    mark_sites(ax)
    ...
    im_th = ax.imshow(residual, cmap='hot', origin='lower', aspect='equal',
                      vmin=0, vmax=th_vmax, interpolation='none')
    ax.set_title('TopHat r=50ft (road width)', fontsize=12,
                 fontweight='bold')
    ax.set_ylabel('TopHat Extraction', fontsize=13, fontweight='bold',
                  rotation=90, labelpad=10)
    strip_axes(ax)
    mark_sites(ax)
    ...
    im_fine = ax.imshow(residual_fine, cmap='hot', origin='lower',
                        aspect='equal', vmin=0, vmax=fine_vmax,
                        interpolation='none')
    ax.set_title('TopHat r=7.5ft / 15ft dia (~1m features)', fontsize=12,
                 fontweight='bold')
    strip_axes(ax)
    mark_sites(ax)
    ...
    try:
        _trend_mid, residual_mid = run_decomposition(
            'tophat', dem, {'size': 10, 'mode': 'white'})
    except Exception:
    ...
    im_mid = ax.imshow(residual_mid, cmap='hot', origin='lower',
                       aspect='equal', vmin=0, vmax=mid_vmax,
                       interpolation='none')
    ax.set_title('TopHat r=25ft / 50ft dia (~5m features)', fontsize=12,
                 fontweight='bold')
    strip_axes(ax)
    mark_sites(ax)
    ...
    try:
        _trend_wide, residual_wide = run_decomposition(
            'tophat', dem, {'size': 40, 'mode': 'white'})
    except Exception:
    ...
    im_wide = ax.imshow(residual_wide, cmap='hot', origin='lower',
                        aspect='equal', vmin=0, vmax=wide_vmax,
                        interpolation='none')
    ax.set_title('TopHat r=100ft / 200ft dia (~30m features)', fontsize=12,
                 fontweight='bold')
    strip_axes(ax)
    mark_sites(ax)
...
def generate_index_map(
    ...
    for tile in all_tiles:
        ...
        ax.text(cx, cy, tid, ha='center', va='center',
    ...
    for name, (sx, sy, marker) in KNOWN_SITES.items():
        ax.plot(sx, sy, marker, markersize=14, markeredgewidth=2,
...
def sites_in_tile(tile: dict) -> list:
    ...
    for name, (sx, sy, _marker) in KNOWN_SITES.items():
        if (tile['x_min'] <= sx <= tile['x_max'] and
                tile['y_min'] <= sy <= tile['y_max']):
...
def main():
    parser = argparse.ArgumentParser(
        description='RESIDUALS: Licking County grid tiling')
    parser.add_argument('--rows', type=str, default=None,
                        help='Row range to process, e.g. "5-8" (0-indexed)')
    parser.add_argument('--cols', type=str, default=None,
                        help='Column range to process, e.g. "6-10" (0-indexed)')
    ...
    logger.info(f"  {len(tiles_with_data)} tiles have LAS data "
                f"(of {len(all_tiles)} total grid cells)")
    ...
    if args.index_only:
        ...
        generate_index_map(all_tiles, n_rows, n_cols, county_bounds,
                           output_dir / 'index_map.png', processed)
        return
    ...
    work_tiles = [
        t for t in tiles_with_data
        if row_min <= t['row'] <= row_max and col_min <= t['col'] <= col_max
    ]
    logger.info(f"Processing {len(work_tiles)} tiles "
    ...
    for i, tile in enumerate(work_tiles):
        ...
        logger.info(f"\n--- {tid} ({i + 1}/{len(work_tiles)}) "
                     f"LAS tiles={tile['n_tiles']} ---")
        ...
        try:
            ...
            if dem is None:
                ...
                logger.info(f"  DEM: {dem.shape} ({dem.size:,} px), "
                            f"elev=[{dem.min():.0f}, {dem.max():.0f}]")

            # --- Load or compute SVF + openness ---
            svf, pos_openness, neg_openness = None, None, None
            if (svf_cache.exists() and pos_open_cache.exists()
                    and neg_open_cache.exists()):
            ...
            if svf is None or pos_openness is None or neg_openness is None:
                ...
                t_svf = time.time() - t0
                logger.info(f"  SVF/openness: {t_svf:.1f}s  "
                            f"SVF=[{svf.min():.3f}, {svf.max():.3f}]")
            ...
            render_tile_png(dem, residual, svf, pos_openness, neg_openness,
                            tid, png_path, site_marks)
            processed_ids.add(tid)
            ...
            for var in ('dem', 'svf', 'pos_openness', 'neg_openness',
                        'residual', '_trend'):
                if var in locals():
                    try:
                        del locals()[var]
        ...
        logger.info(f"  Tile time: {time.time() - t_tile_start:.1f}s  |  "
    ...
    # Generate index map
    generate_index_map(all_tiles, n_rows, n_cols, county_bounds,
                       output_dir / 'index_map.png', all_processed)
    ...
    meta = {
        ...
        'known_sites': {
            ...
            for name, xy in [('Hopewell Road', (HR_X, HR_Y)),
                             ('Great Circle', (GC_X, GC_Y))]
        },
        'crs': CRS,
    ...
    logger.info(f"\nDone. {len(all_processed)} tiles processed in "
