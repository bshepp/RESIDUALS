...
def decompose_bilateral(
    dem: np.ndarray, d: int = 9, sigma_color: float = 75, sigma_space: float = 75
) -> tuple:
    """
...
def decompose_wavelet_dwt(dem: np.ndarray, wavelet: str = "db4", level: int = 3) -> tuple:
    """
...
@register_decomposition(
    ...
    param_ranges={"operation": ["opening", "closing", "average"], "size": [5, 10, 20, 50, 100]},
    preserves="features matching structuring element shape",
    destroys="features smaller than element",
)
def decompose_morphological(dem: np.ndarray, operation: str = "opening", size: int = 10) -> tuple:
    """
...
def decompose_tophat(dem: np.ndarray, size: int = 20, mode: str = "white") -> tuple:
    """
...
def decompose_polynomial(dem: np.ndarray, degree: int = 2) -> tuple:
    ...
    if degree == 1:
        ...
        A = np.column_stack(
            [np.ones_like(X_flat), X_flat, Y_flat, X_flat**2, X_flat * Y_flat, Y_flat**2]
        )
    else:  # degree 3
        A = np.column_stack(
            [
