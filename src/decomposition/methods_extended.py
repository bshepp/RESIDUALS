...
from skimage.morphology import diamond, disk, ellipse, rectangle, square
...
def _morphological_decompose(dem_filled: np.ndarray, selem: np.ndarray, operation: str) -> tuple:
...
def decompose_gaussian_anisotropic(
    dem: np.ndarray, sigma_x: float = 10, sigma_y: float = 10
) -> tuple:
    """
...
def decompose_dog(dem: np.ndarray, sigma_low: float = 2, sigma_high: float = 10) -> tuple:
    """
...
def decompose_dog_multiscale(
    dem: np.ndarray, sigma_ratio: float = 1.6, n_scales: int = 4, base_sigma: float = 1.0
) -> tuple:
    """
...
def decompose_morphological_square(
    dem: np.ndarray, operation: str = "opening", size: int = 10
) -> tuple:
    """
...
def decompose_morphological_rect(
    dem: np.ndarray, operation: str = "opening", width: int = 20, height: int = 5
) -> tuple:
    """
...
def decompose_morphological_diamond(
    dem: np.ndarray, operation: str = "opening", radius: int = 10
) -> tuple:
    """
...
def decompose_morphological_ellipse(
    dem: np.ndarray, operation: str = "opening", width: int = 20, height: int = 10
) -> tuple:
    """
...
def decompose_morphological_gradient(dem: np.ndarray, size: int = 5, shape: str = "disk") -> tuple:
    """
...
def decompose_anisotropic_diffusion(
    dem: np.ndarray, iterations: int = 10, kappa: float = 50, gamma: float = 0.1
) -> tuple:
    """
...
def _rolling_ball_core(dem: np.ndarray, radius: int) -> np.ndarray:
    ...
    ball = np.where(dist_sq <= radius**2, np.sqrt(radius**2 - dist_sq), 0)
...
@register_decomposition(
    ...
    param_ranges={"window_size": [21, 31, 51, 101], "degree": [1, 2, 3]},
    preserves="local deviations from local polynomial trend",
    destroys="features smoother than local polynomial",
)
def decompose_local_polynomial(dem: np.ndarray, window_size: int = 51, degree: int = 2) -> tuple:
    """
    ...
    if degree == 1:
        ...
        sobel(dem_filled, axis=1, mode="reflect") / 8
        sobel(dem_filled, axis=0, mode="reflect") / 8
...
def decompose_guided(dem: np.ndarray, radius: int = 8, eps: float = 0.01) -> tuple:
    """
    ...
    mean_g = uniform_filter(guide, size=2 * radius + 1)
    mean_p = uniform_filter(p, size=2 * radius + 1)
    corr_gp = uniform_filter(guide * p, size=2 * radius + 1)
    corr_gg = uniform_filter(guide * guide, size=2 * radius + 1)
...
def decompose_wavelet_biorthogonal(
    dem: np.ndarray, wavelet: str = "bior3.5", level: int = 3
) -> tuple:
    """
...
def decompose_wavelet_reverse_biorthogonal(
    dem: np.ndarray, wavelet: str = "rbio3.5", level: int = 3
) -> tuple:
    """
