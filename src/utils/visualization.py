...
def visualize_decomposition(
    ...
    im2 = axes[2].imshow(residual, cmap="RdBu_r", origin="lower", vmin=-vmax, vmax=vmax)
...
def visualize_differential(
    diff: np.ndarray, title: str, output_path: Optional[str] = None, percentile_clip: float = 99
) -> None:
    """
...
def visualize_top_results(
    ...
    for i, (key, score, analysis) in enumerate(ranked_results[:top_n]):
        ...
        fig, axes = plt.subplots(
            1, 2, figsize=(12, 5), gridspec_kw={"width_ratios": [2, 1], "wspace": 0.1}
        )

        # Differential image
        vmax = np.percentile(np.abs(diff), 99)
        im = axes[0].imshow(
            diff, cmap="RdBu_r", origin="lower", vmin=-vmax, vmax=vmax, aspect="auto"
        )
...
def visualize_results(
    ...
    if has_ground_truth:
        ...
        shaded = np.cos(altitude) * np.sin(np.arctan(slope)) * np.cos(azimuth - aspect) + np.sin(
            altitude
        ) * np.cos(np.arctan(slope))
    ...
    for decomp_name in decomp_names:
        all_residuals = [
            results[decomp_name][u]["residual"] for u in upsamp_names if u in results[decomp_name]
        ]
        if all_residuals:
    ...
    for i, decomp_name in enumerate(decomp_names):
        ...
        for j, upsamp_name in enumerate(upsamp_names):
            ...
            if has_gt_comparison and gt_gradient is not None:
                ...
                gt_pct = np.percentile(gt_gradient, 99)
                gt_norm = gt_gradient / gt_pct if gt_gradient.max() > 0 else gt_gradient
                ...
                composite[y_start : y_start + img_h, x_start_diff : x_start_diff + img_w] = (
                    diff_colored
                )
        ...
        for i, decomp_name in enumerate(decomp_names):
            ...
            all_res = np.stack(
                [
                    ...
                    for u in upsamp_names
                    if u in results[decomp_name]
                ],
                axis=0,
            )
    ...
    plt.savefig(filepath, dpi=dpi, bbox_inches="tight", pad_inches=0.05, facecolor="white")
    plt.close()
