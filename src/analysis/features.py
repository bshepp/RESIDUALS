...
def analyze_features(diff: np.ndarray, original_dem: np.ndarray = None) -> Dict[str, Any]:
    """
...
def detect_linear_features(
    diff: np.ndarray, canny_sigma: float = 2.0, num_peaks: int = 20
) -> Dict[str, Any]:
    """
...
def analyze_frequency_content(diff: np.ndarray) -> Dict[str, float]:
    ...
    dominant_freq_r = np.sqrt((peak_idx[0] - center[0]) ** 2 + (peak_idx[1] - center[1]) ** 2)
...
def rank_for_features(
    differentials: Dict[str, np.ndarray], analyses: Dict[str, Dict[str, Any]]
...
def generate_analysis_report(
    ranked_results: List[Tuple[str, float, Dict[str, Any]]], top_n: int = 20
) -> str:
    """
