...
def compute_differential(
    result_a: np.ndarray, result_b: np.ndarray, method: str = "subtract"
) -> np.ndarray:
    """
    ...
    for decomp_name in decomposition_methods:
        ...
        for upsamp_name in upsampling_methods:
            ...
            try:
                ...
                results[decomp_name][upsamp_name] = {"trend": trend_up, "residual": residual_up}

                # Optionally save to disk
                if output_dir:
                    output_dir.mkdir(parents=True, exist_ok=True)
                    np.save(output_dir / f"{decomp_name}_{upsamp_name}_trend.npy", trend_up)
                    np.save(output_dir / f"{decomp_name}_{upsamp_name}_residual.npy", residual_up)
...
def compute_all_differentials(
    ...
    logger.info(
        f"Computing differentials for {len(all_combos)} combinations "
...
def compute_method_differentials(
    results: Dict[str, Dict[str, Dict[str, np.ndarray]]], differential_method: str = "subtract"
) -> Dict[str, Dict[str, np.ndarray]]:
    """
