...
def upsample_area(dem: np.ndarray, scale: int = 2) -> np.ndarray:
    ...
    return cv2.resize(dem_filled.astype(np.float32), (new_w, new_h), interpolation=cv2.INTER_AREA)
...
def upsample_linear_exact(dem: np.ndarray, scale: int = 2) -> np.ndarray:
    ...
    return cv2.resize(
        dem_filled.astype(np.float32), (new_w, new_h), interpolation=cv2.INTER_LINEAR_EXACT
    )
...
def upsample_sinc_hamming(dem: np.ndarray, scale: int = 2, kernel_size: int = 8) -> np.ndarray:
    """
...
def upsample_sinc_blackman(dem: np.ndarray, scale: int = 2, kernel_size: int = 8) -> np.ndarray:
    """
...
def upsample_cubic_catmull_rom(dem: np.ndarray, scale: int = 2) -> np.ndarray:
    ...
    def catmull_rom_kernel(t):
        ...
        t3 = t2 * t
        return np.where(
            t <= 1, 1.5 * t3 - 2.5 * t2 + 1, np.where(t <= 2, -0.5 * t3 + 2.5 * t2 - 4 * t + 2, 0)
        )
...
def upsample_edge_directed(dem: np.ndarray, scale: int = 2) -> np.ndarray:
    ...
    lanczos = cv2.resize(
        dem_filled.astype(np.float32), (new_w, new_h), interpolation=cv2.INTER_LANCZOS4
    )
...
def upsample_regularized(dem: np.ndarray, scale: int = 2, lambda_reg: float = 0.01) -> np.ndarray:
    """
