...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("bestof_experiment.log"), logging.StreamHandler()],
)
...
BESTOF_DECOMPOSITIONS = [
    ...
    ("gaussian", {"sigma": 5}, "Fine local features (small-sigma Gaussian)"),
    ("gaussian", {"sigma": 50}, "Regional trend removal (large-sigma Gaussian)"),
    # --- Edge-preserving ---
    ("bilateral", {"d": 9, "sigma_color": 75, "sigma_space": 75}, "Edge-preserving smoothing"),
    (
        "anisotropic_diffusion",
        {"iterations": 10, "kappa": 50, "gamma": 0.1},
        "Edge-preserving via iterative diffusion",
    ),
    ("guided", {"radius": 8, "eps": 0.01}, "Self-guided edge-preserving filter"),
    # --- Wavelet family ---
    ("wavelet_dwt", {"wavelet": "db4", "level": 2}, "Wavelet high-frequency detail (shallow)"),
    ("wavelet_dwt", {"wavelet": "db4", "level": 4}, "Wavelet low-frequency structure (deep)"),
    (
        "wavelet_biorthogonal",
        {"wavelet": "bior3.5", "level": 3},
        "Linear-phase wavelet multi-scale",
    ),
    # --- Morphological family ---
    ("morphological", {"operation": "opening", "size": 15}, "Circular structuring element (disk)"),
    ("morphological_square", {"operation": "opening", "size": 10}, "Axis-aligned square element"),
    (
        "morphological_ellipse",
        {"operation": "opening", "width": 20, "height": 10},
        "Elongated elliptical element",
    ),
    (
        "morphological_rect",
        {"operation": "opening", "width": 20, "height": 5},
        "Directional rectangular element (linear features)",
    ),
    ("morphological_gradient", {"size": 5, "shape": "disk"}, "Morphological edge detection"),
    # --- Top-hat family ---
    ("tophat", {"size": 20, "mode": "white"}, "Small bright features (white top-hat)"),
    ("tophat_combined", {"size": 20}, "Both bright and dark small features"),
    # --- Rolling ball ---
    ("rolling_ball", {"radius": 25}, "Fine features (small ball)"),
    # --- Multi-scale / bandpass ---
    ("dog", {"sigma_low": 2, "sigma_high": 10}, "Bandpass: features at intermediate scales"),
    (
        "dog_multiscale",
        {"sigma_ratio": 1.6, "n_scales": 4, "base_sigma": 1.0},
        "Multi-scale blob detection",
    ),
    ("log", {"sigma": 5}, "Laplacian of Gaussian blob detection"),
    # --- Polynomial trend removal ---
    ("polynomial", {"degree": 2}, "Quadratic regional trend removal"),
]
...
def run_bestof(
    ...
    """
    from src.analysis import analyze_features, generate_analysis_report, rank_for_features
    ...
    logger.info(
        f"Best-of pipeline: {n_decomp} decompositions x {n_upsamp}"
        f" upsamplings = {total} combinations"
    )
        ...
        json.dump(
            {
                ...
                "upsamplings": [
                    {"name": n, "params": p, "description": d} for n, p, d in BESTOF_UPSAMPLINGS
                ],
...
Examples:
    ...
    parser.add_argument("--output", type=str, default="results/bestof", help="Output directory")
