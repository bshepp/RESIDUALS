...
def crop_to_site(dem, meta, site_name, radius=SITE_CROP_RADIUS):
    ...
    if meta is None or 'sites' not in meta or site_name not in meta['sites']:
        raise ValueError(f"Site '{site_name}' not found in metadata.  "
...
def plot_site_comparison(dem, site_info, results, output_path, site_name):
    ...
    fig.suptitle(
        f"RESIDUALS: {desc}\n"
        f"Raw hillshade (left) vs. decomposition residuals (right)",
...
def plot_grid(dem, dem_label, results, output_path, scale, upsamp_name):
    ...
    for i, (name, _params, label) in enumerate(DECOMP_METHODS):
        ...
        im = ax.imshow(data["residual"], cmap="RdBu_r", origin="lower",
        ...
        im = ax.imshow(up, cmap="RdBu_r", origin="lower", aspect="equal",
                        vmin=-vmax_up, vmax=vmax_up)
        ax.set_title(f"Residual {scale}x ({upsamp_name})", fontsize=9)
        ax.set_xticks([]); ax.set_yticks([])
        plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)

    fig.suptitle("RESIDUALS: Label-Free Feature Enhancement via Signal Decomposition",
...
def run_demo(dem_path, output_path, scale=2, site=None, meta_path=None):
    ...
    if site:
        if meta is None:
            print(f"ERROR: --site requires a metadata JSON with site coordinates.")
            print(f"       Use --meta path/to/metadata.json")
            sys.exit(1)
...
Examples:
    ...
    parser.add_argument("--dem", type=Path, default=default_dem,
                        help=f"Path to DEM file.  Default: {default_dem}")
    parser.add_argument("--meta", type=str, default=default_meta,
                        help="Path to JSON metadata with site coordinates")
    parser.add_argument("--output", type=Path, default=Path("demo_output.png"),
